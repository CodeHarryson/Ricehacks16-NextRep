...
| Location/battle | Stage 3 location publishing, nearby map, proximity challenge handshake, shared squat configuration, challenge-aware camera navigation, and timed session control are implemented. Rep tracking remains local per device; battle scoring, result synchronization, and performance persistence remain planned. |
