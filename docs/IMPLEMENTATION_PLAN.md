...
| Location/battle | Stage 3 Step 2 location publishing, nearby demo map, and the proximity challenge handshake are implemented with Tiger Data/PostGIS/Timescale presence storage, server-owned radius/distance, expiry cleanup, privacy-safe markers, and receiver-only accept/decline. Shared workout configuration, battle scoring, and resolution remain planned. |
