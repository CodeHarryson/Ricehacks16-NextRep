...
| Location/battle | Stage 3 location publishing, nearby map, proximity challenge handshake, shared squat configuration, challenge-aware camera navigation, timed session control, server-validated result submission, and result polling/resolution are implemented. Rep tracking remains local per device; battle rewards and advanced leaderboards remain planned. |
