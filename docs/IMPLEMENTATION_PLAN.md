...
| Location/battle | Stage 3 Step 2 location publishing and nearby demo map are implemented with Tiger Data/PostGIS/Timescale presence storage, server-owned radius/distance, expiry cleanup, and privacy-safe markers. Battles, check-in, acceptance, settings lock, and resolution remain planned. |
